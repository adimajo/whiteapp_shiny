# Lint scripts
docOrg <- lapply(dir("R", pattern = "*.R", recursive = TRUE, full.names = TRUE),
                 readLines)
styler::style_pkg()
docNew <- lapply(dir("R", pattern = "*.R", recursive = TRUE, full.names = TRUE),
                 readLines)
  ...
  for (doc_new in seq(length(docOrg))) {
    if (!identical(docOrg[[doc_new]], docNew[[doc_new]])) {
      for (line_new in seq(1, length(docOrg[[doc_new]]))) {
        if (!identical(docOrg[[doc_new]][[line_new]],
                       docNew[[doc_new]][[line_new]])) {
